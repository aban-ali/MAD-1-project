    ...
    if viewer=="admin":
        ...
        if ans:
            if password==ans.password:
                ...
                if c.blacklist==1:
                    return render_template("creator.html",user="creator", msg="Your ID has been block because of excessive flagged content !")
                n=c.name
                myalbm=Album.query.filter_by(c_no=c.c_id).all()
                s=Songs.query.filter_by(c_id=c.c_id).order_by(Songs.song_id.desc()).all()
...
def change_admin(ch):
    ...
    if ch=="register":
        if Admin.query.first():
            return render_template("changes_admin.html",types=ch,view="hidden", Change="Register", msg="Admin already exists. Try to change aadmin details")
        else:
...
def signup():
    ...
    if user=="creator":
        if not (Creator.query.filter_by(id=uid).first()):
            new=Creator(id=uid,password=pswd,name=name,blacklist=0,flaged=0)
            db.session.add(new)
            db.session.commit()
            return render_template("creator.html", user="creator")
    else:
        if not (Users.query.filter_by(id=uid).first()):
            new=Users(id=uid,password=pswd,name=name)
            db.session.add(new)
            db.session.commit()
            return render_template("user.html", user="user")
    return render_template("signup.html", msg="User name already exits!!! Please chose a differet one")
